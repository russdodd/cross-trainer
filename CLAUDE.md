...
- `TrackingFeedbackService` (`providedIn: 'root'`) wraps `localStorage` key `cross-trainer.tracking-feedback.v1` — a JSON array of `TrackingFeedbackRecord` (`timestamp`, `rating`, `level`, `grade`, `bandFilter`, `solutionRevealed`, `solutionMatch`, `scrambleIndex`, `scramble`). `toCsv()` emits those as columns in that order.
