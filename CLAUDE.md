...
- **`solutionMatch` (`same` / `different` / null)** records whether the user's own cross matched the suggested line. Its buttons appear only once the solution has been revealed (you can't compare to a suggestion you haven't seen). Unanswered ⇒ null ⇒ blank CSV cell. Added after the key was created, so old rows simply lack it; no migration, no key bump.
- **Neither `rating` nor `solutionMatch` is individually required** — `canSubmit` gates on *at least one* being answered (`selectedRating !== null || selectedMatch !== null`), so a vote can be just a difficulty opinion, just a same/different judgement, or both. `rating` is therefore `Rating | null` in the record type, mirroring how `solutionMatch` was always optional; a rating-only submission still works exactly as before.
